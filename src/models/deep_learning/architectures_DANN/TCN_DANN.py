"""NeuroCap – TCN_DANN sur datasets augmentés (A, B, C, D, FULL) + LOSO"""
import sys
...
class TCNBlock(nn.Module):
    def __init__(self, in_ch, out_ch, dilation=1, dropout=0.2):
        super().__init__()
        pad = (7 - 1) * dilation // 2
        self.conv1 = nn.Conv1d(in_ch, out_ch, 7, padding=pad, dilation=dilation)
        self.bn1 = nn.BatchNorm1d(out_ch)
        self.conv2 = nn.Conv1d(out_ch, out_ch, 7, padding=pad, dilation=dilation)
...
class TCN_DANN(nn.Module):
    """
    TCN + DANN.
    Convolutions dilatées avec résidus (feat_dim=128) → label_head + domain_head.
    """
