"""
=============================================================================
NeuroCap — Baseline ML Régression + SMOTE (feat15)
baseline_ML_regression_smote.py
=============================================================================
MÊME MODÈLES DE RÉGRESSION que baseline_ML_regression.py
  SVR · Random Forest · XGBoost · LightGBM
  → prédisent le score CONTINU 0-10 (MAE, RMSE, R² maintenus)

DIFFÉRENCES vs baseline_ML_regression.py :
  3 techniques anti-imbalance appliquées dans la boucle LOSO :

  ① sample_weight
     Upweight les échantillons High (score ≥ 5.0) pendant le fit()
     ratio = n_Low / n_High ≈ 2.45 pour stress
     → les modèles ne prédisent plus systématiquement la moyenne
     Ref : He & Garcia (2009). IEEE TKDE, 21(9), 1263-1284.

  ② SMOTE sur fold TRAIN uniquement
     Génère des échantillons synthétiques High (score ≥ 5.0)
     par interpolation KNN dans l'espace des features
     y continu assigné par tirage depuis la distribution High originale
     → Plus d'exemples High → le modèle apprend mieux la région High
     Ref : Chawla et al. (2002). JAIR, 16, 321-357.

  ③ Seuil Youden optimal
     seuil_optimal = argmax(Sensitivity + Specificity − 1) sur fold train
     remplace le seuil fixe 5.0 pour la décision binaire
     → maximise simultanément Sensitivity ET Specificity
     Ref : Youden (1950). Cancer, 3(1), 32-35.

EVALUATION :
  y_true  : scores continus 0-10
  y_pred  : scores continus 0-10 (sortie régression, inchangé)
  threshold: seuil Youden calculé sur fold train (échelle 0-10)

  ┌──────────────────────────────────────────────────────────┐
  │  Régression : MAE, RMSE, R²  (threshold indépendant)    │
  │  Classif.   : AUC, Sens, Spec, MCC  (threshold=Youden) │
  └──────────────────────────────────────────────────────────┘

COMPARAISON (même threshold=5.0 stocké aussi pour comparaison directe) :
  baseline_ML_regression.py  → threshold=5.0 fixe, pas de repondération
  THIS SCRIPT                → SMOTE + sample_weight + seuil Youden

SORTIES :
  reports/Regression/Baseline/feat15_smote/{conc,stress}/
    comparison_all.png · results_{target}.json · report_{target}.txt
...
[2] He & Garcia (2009). Learning from Imbalanced Data. IEEE TKDE, 21(9).
    DOI: 10.1109/TKDE.2009.30
[3] Youden (1950). Index for rating diagnostic tests. Cancer, 3(1), 32-35.
    DOI: 10.1002/1097-0142(1950)3:1<32
[4] Chicco & Jurman (2020). Advantages of MCC over F1. BMC Genomics, 21:6.
    DOI: 10.1186/s12864-019-6413-7
[5] Lotte et al. (2018). Review EEG-BCI classification. J.Neural Eng., 15.
...
try:
    ...
    HAS_SMOTE = False
    print("[WARNING] imbalanced-learn non installé → pip install imbalanced-learn")
    print("          Fallback : sample_weight uniquement (sans SMOTE)")

# ─── Chemins projet ──────────────────────────────────────────────────────────
PROJECT_ROOT = Path(__file__).resolve().parents[3]
AUG_ROOT    = PROJECT_ROOT / 'data'    / 'Regression' / 'augmented'
FEAT_ROOT   = PROJECT_ROOT / 'Features'
OUTPUT_ROOT = PROJECT_ROOT / 'reports' / 'Regression' / 'Baseline' / 'feat15_smote'
MODEL_ROOT  = PROJECT_ROOT / 'models'  / 'Regression' / 'Baseline' / 'feat15_smote'
...
# SMOTE aggrave les résultats sur stress (scores trop variables inter-sujets)
# → désactivé pour stress, conservé pour conc uniquement
SMOTE_TARGETS = {'conc'}
...
# ─────────────────────────────────────────────────────────────────────────────
# SEUIL YOUDEN OPTIMAL (sur sortie de régression — échelle 0-10)
# Ref: Youden (1950). Cancer, 3(1), 32-35.
# ─────────────────────────────────────────────────────────────────────────────
def find_youden_threshold(y_true_bin, y_pred_continuous):
    """
    Seuil optimal J = argmax(Sensitivity + Specificity − 1).
    Calculé sur la courbe ROC du fold TRAIN.
    Le seuil est dans la même échelle que y_pred (0-10).
...
def loso_cv_smote(X, y, sids, model, model_name='', target='conc'):
    """
    LOSO cross-validation avec 3 techniques anti-imbalance.

    Les MODÈLES restent des RÉGRESSEURS (prédisent scores 0-10).
    L'imbalance est corrigée au niveau de l'ENTRAÎNEMENT :

    Par fold :
      1. Séparation train / test stricte par sujet
      2. Normalisation StandardScaler (fit sur train, transform sur test)
      3. SMOTE sur (X_train, y_train_binary) → génère des High synthétiques
         y continu des synthétiques = tirage depuis distribution High originale
      4. sample_weight = n_Low/n_High ≈ 2.45 sur données augmentées
      5. Seuil Youden calculé sur prédictions fold TRAIN (ROC curve)
      6. Évaluation sur fold TEST avec seuil Youden

    y_true et y_pred restent en échelle 0-10 → MAE/RMSE/R² inchangés.
    La binarisation utilise le seuil Youden (pas 5.0 fixe).
    ...
    for ts in unique_s:
        ...
        # ── Labels binaires pour SMOTE et Youden ─────────────────────────────
        y_tr_bin = bin_scores(y_tr, THRESHOLD_DEFAULT)
        n_high   = int(np.sum(y_tr_bin == 1))
        n_low    = int(np.sum(y_tr_bin == 0))

        # ── SMOTE sur fold TRAIN (jamais sur test, jamais sur stress) ───────────
        # Désactivé pour stress : scores trop variables inter-sujets → SMOTE aggrave
        # Activé pour conc uniquement (classes plus stables)
        # [Ref 1] Chawla et al. (2002)
        if HAS_SMOTE and n_high >= 6 and target in SMOTE_TARGETS:
            ...
            # Assigner des y continus aux échantillons synthétiques
            n_orig = len(X_tr)
            y_aug  = np.zeros(len(X_aug))
            y_aug[:n_orig] = y_tr  # originaux : y exact conservé
            ...
            for i in range(n_orig, len(X_aug)):
                if y_bin_aug[i] == 1:
                    y_aug[i] = rng.choice(y_high_dist)
                else:
                    y_aug[i] = rng.choice(y_low_dist)
        ...
        # ── sample_weight : upweight les High  ───────────────────────────────
        # ratio = n_Low / n_High plafonné à 2.5 — He & Garcia (2009) recommandent ≤ 3.0
        ...
        # ── Entraînement — Optuna tune sur X_tr_s (pre-SMOTE), fit sur X_aug ─
        ...
        # ── Prédictions train (pour Youden) et test ──────────────────────────
        y_pred_tr = mc.predict(X_tr_s)   # sur train original (pas augmenté)
        y_pred_te = mc.predict(X_te_s)   # sur test

        # ── Seuil Youden sur fold TRAIN ──────────────────────────────────────
        # Calculé sur les prédictions continues (0-10) du fold train
        # [Ref 3] Youden (1950)
        youden = find_youden_threshold(y_tr_bin, y_pred_tr)
        youden_thresholds.append(youden)

        # ── Métriques par fold ───────────────────────────────────────────────
        # threshold=youden : décision binaire optimale
        # MAE / RMSE / R² restent exacts (indépendants du threshold)
        ...
        # Aussi stocker les métriques à threshold=5.0 pour comparaison directe
    ...
    if not yt_all:
        ...
        # Métriques à threshold=5.0 pour comparaison directe avec baseline
...
# ─────────────────────────────────────────────────────────────────────────────
# EXTRACTION FEAT15 À LA VOLÉE (identique à baseline_ML_regression.py)
# ─────────────────────────────────────────────────────────────────────────────
...
def plot_youden_roc(yt, yp, model_name, exp, youden_thresh, out_dir):
    """Courbe ROC avec annotation du seuil Youden optimal."""
    yt_a = np.array(yt)
    yp_a = np.array(yp)
    ...
    fpr, tpr, thresholds = sk_roc_curve(y_bin, yp_a)
    from sklearn.metrics import auc as sk_auc
    auc_val = sk_auc(fpr, tpr)

    # Point Youden sur la courbe
    youden_bin_pred = bin_scores(yp_a, youden_thresh)
    tp_y = int(np.sum((youden_bin_pred == 1) & (y_bin == 1)))
    fp_y = int(np.sum((youden_bin_pred == 1) & (y_bin == 0)))
    tn_y = int(np.sum((youden_bin_pred == 0) & (y_bin == 0)))
    fn_y = int(np.sum((youden_bin_pred == 0) & (y_bin == 1)))
    ...
    # Marquer seuil fixe 5.0
    bin5 = bin_scores(yp_a, 5.0)
    tp5 = int(np.sum((bin5 == 1) & (y_bin == 1)))
    fp5 = int(np.sum((bin5 == 1) & (y_bin == 0)))
    tn5 = int(np.sum((bin5 == 0) & (y_bin == 0)))
    fn5 = int(np.sum((bin5 == 0) & (y_bin == 1)))
    sens5 = tp5 / max(tp5 + fn5, 1)
    spec5 = tn5 / max(tn5 + fp5, 1)
    ax.scatter([1 - spec5], [sens5], marker='D', s=120, color='gray', zorder=4,
               label=f'Seuil 5.0  Sens={sens5:.2f}  Spec={spec5:.2f}')
    ...
    ax.set_title(f'ROC + seuil Youden — {model_name} (Exp.{exp})\n[Ref 3] Youden 1950')
    ax.legend(fontsize=8)
    ax.grid(alpha=0.3)
    plt.tight_layout()
...
def plot_folds(fold_metrics, model_name, exp, out_dir):
    ...
    fig.suptitle(f'Métriques LOSO — {model_name} (Exp.{exp})  [SMOTE+sw+Youden]', fontsize=11)
    specs = [
    ...
    for ax, key, ylabel, color, show_std5 in specs:
        ...
        ax.plot(folds, vals, 'o-', color=color, lw=2, markersize=5, label=f'Youden μ={np.mean(vals):.3f}')
...
def plot_comparison(results, target, out_dir):
    """Barplot MAE / AUC / MCC / Sensitivity par expérience × modèle."""
    ...
    for idx, (metric, ylabel, lower_is_better) in enumerate(metrics_info):
        ax = axes[idx]
    ...
    plt.suptitle(
        f'NeuroCap — Baseline SMOTE+sample_weight+Youden — {target} (feat15)\n'
...
def plot_comparison_smote_vs_baseline(results_smote, results_baseline, target, out_dir):
    """Comparaison directe SMOTE vs Baseline pour Sensitivity, Specificity, MCC."""
    ...
    x = np.arange(len(exps_ok))
    w = 0.10
    colors_smote    = list(COLORS.values())
    colors_baseline = [c + '88' for c in colors_smote]  # transparence
    ...
    for idx, (metric, ylabel) in enumerate([
        ...
        ax = axes[idx]
        for i, mn in enumerate(MODELS):
            # Valeurs SMOTE (avec Youden)
            vals_s = [results_smote[e].get(mn, {}).get(metric, 0) for e in exps_ok]
            # Valeurs baseline (threshold=5.0)
            vals_b = [results_baseline.get(e, {}).get(mn, {}).get(metric, 0)
                      for e in exps_ok]

            ax.bar(x + (2*i)*w,     vals_b, w, label=f'{mn} (baseline)',
                   color=COLORS.get(mn, '#7F8C8D'), alpha=0.4, hatch='//')
            ax.bar(x + (2*i+1)*w,   vals_s, w, label=f'{mn} (SMOTE)',
                   color=COLORS.get(mn, '#7F8C8D'), alpha=0.9)
    ...
    plt.suptitle(f'Impact SMOTE+sample_weight+Youden vs Baseline — {target}', fontsize=12)
    plt.tight_layout(rect=[0, 0, 1, 0.92])
...
def run_professional_analysis(
    ...
    dl  = delong_auc_ci(_bin(yt_a, THRESHOLD_DEFAULT), yp_a)
...
# ÉVALUATION VAL / TEST
# ─────────────────────────────────────────────────────────────────────────────
def eval_split(model, scaler, split, aug_dir, feat_dir, youden_thresh):
    ...
    yh = model.predict(Xs)
    m  = compute_full_metrics(y, yh, threshold=youden_thresh)
    m['n_samples'] = len(y)
    return m
...
def run_target(target, baseline_results=None):
    ...
    if not aug_dir.exists():
        print(f"  [SKIP] {aug_dir} introuvable — lancez pipeline_regression.py d'abord")
        return {}
    ...
    for exp in EXPERIMENTS:
        ...
        print(f"\n{'='*70}")
        print(f"  {target.upper()}  Exp.{exp} — feat15 + SMOTE + sample_weight...")
        t_feat = time.time()
        X_raw  = np.load(x_raw)
        X      = extract_feat15(X_raw)
        y      = np.load(yf)
        ...
        print(f"  feat15 {X.shape} | {len(np.unique(sids))} sujets | "
              f"Low={n_low_g} ({100*n_low_g/len(y):.0f}%) "
        ...
        for mn, model in MODELS.items():
            ...
            sc.fit(X)
            mf = clone(model)
            y_bin_tr = bin_scores(y, THRESHOLD_DEFAULT)
            n_high_tr = int(np.sum(y_bin_tr == 1))
            n_low_tr  = int(np.sum(y_bin_tr == 0))
            sw_all = np.where(y_bin_tr == 1, min(float(n_low_tr) / max(n_high_tr, 1), 2.5), 1.0)
            try:
                mf.fit(sc.transform(X), y, sample_weight=sw_all)
            except TypeError:
                mf.fit(sc.transform(X), y)

            vm = eval_split(mf, sc, 'val',  aug_dir, feat_dir, youden_mean)
            tm = eval_split(mf, sc, 'test', aug_dir, feat_dir, youden_mean)
            if vm:
        ...
        if len(exp_fold_r2_all) >= 2:
            ...
            plot_wilcoxon_heatmap(wilcox_r2,  list(exp_fold_r2_all.keys()),
                                  'R²',   f'Exp.{exp} SMOTE', od_exp / 'wilcoxon_r2.png')
            plot_wilcoxon_heatmap(wilcox_auc, list(exp_fold_auc_all.keys()),
                                  'AUC',  f'Exp.{exp} SMOTE', od_exp / 'wilcoxon_auc.png')
    ...
    # Figure comparative SMOTE vs baseline (si données disponibles)
    ...
    with open(json_path, 'w') as f:
        json.dump({
            ...
            'config': {
                ...
                'models':          'regressors (SVR, RF, XGB, LGB)',
                'references': {
                    ...
                    'eeg_bci':        'Lotte et al. (2018) J.Neural Eng. 15:031005',
                },
            },
    ...
    if best_cfg:
        ...
        # SMOTE sur données complètes d'entraînement
        ...
        if HAS_SMOTE and n_h >= 6:
            ...
            y_aug_b[:len(yb)] = yb
            y_high_d = yb[y_bin_b == 1]
            y_low_d  = yb[y_bin_b == 0]
            rng = np.random.default_rng(42)
            for i in range(len(yb), len(X_aug_b)):
                y_aug_b[i] = rng.choice(y_high_d) if y_bin_aug_b[i] == 1 else rng.choice(y_low_d)
        else:
        ...
        joblib.dump(mf, mdl_root / f'{mn_safe}_{target}_smote.joblib')
        joblib.dump(sc, mdl_root / f'{mn_safe}_{target}_scaler_smote.joblib')
        print(f"  Modèle : {mn_safe}_{target}_smote.joblib")
...
def _write_text_report(out_root, target, all_results, best_cfg, best_r2, best_auc):
    with open(out_root / f'report_{target}.txt', 'w', encoding='utf-8') as f:
        f.write(f"NeuroCap — Baseline SMOTE+sample_weight+Youden (feat15) — {target}\n")
        f.write("=" * 90 + "\n")
...
def main():
    print("=" * 75)
    print("NeuroCap — Baseline ML Régression + SMOTE + sample_weight + Youden")
    print("Modèles : SVR · RF · XGBoost · LightGBM (régresseurs)")
    print()
    print("Références scientifiques :")
    ...
    print("  [5] Lotte et al. (2018) EEG-BCI    — J.Neural Eng. 15")
    print("=" * 75)
    ...
    # Charger les résultats baseline pour le graphe comparatif
    baseline_results = {}
    for target in ['conc', 'stress']:
        bp = PROJECT_ROOT / 'reports' / 'Regression' / 'Baseline' / 'feat15' / target / f'results_{target}.json'
        if bp.exists():
            with open(bp) as f:
                data = json.load(f)
            baseline_results[target] = data.get('loso', {})
            print(f"  Résultats baseline chargés pour {target} (comparaison activée)")
        else:
            print(f"  [INFO] Pas de résultats baseline pour {target} — "
                  f"lancez baseline_ML_regression.py d'abord")
