"""
NeuroCap – EEGNet Layer Replacement RÉGRESSION (Olesen 2020, TL-3) ★ RECOMMANDÉ
=================================================================================
Stratégie  : remplacer conv1+bn1 (adaptation temporelle) + geler dw→spw + dégeler FC.
...
Sorties      : reports/Regression/TL/EEGNet_LayerReplacement/{target}/{exp}/

Principe (Olesen 2020) :
  1. Charger EEGNet pré-entraîné sur la population.
  2. Geler toutes les couches (patterns spectraux génériques conservés).
  3. Remplacer conv1+bn1 par de nouvelles couches non initialisées
     → adaptation du filtrage temporel au profil neurophysiologique du sujet.
  4. Dégeler FC → adapter la tête de régression.
  5. Fine-tuner sur quelques minutes de données de calibration (X_val).

Avantage : conserve les patterns β/α/θ appris sur la population tout en
adaptant le filtrage temporel et la sortie au sujet cible.
"""
...
# ── Stratégie TL-3 : Layer Replacement ────────────────────────────────────────
def apply_layer_replacement(model):
    """
    Geler tout, remplacer conv1+bn1 (nouveaux paramètres → requires_grad=True),
    dégeler FC — Olesen 2020.
    """
    for p in model.parameters():
        p.requires_grad = False
    # Nouvelles couches avec Kaiming init pour meilleur point de départ
    model.conv1 = nn.Conv2d(1, 8, (1, 64), padding=(0, 32), bias=False)
    model.bn1   = nn.BatchNorm2d(8)
    nn.init.kaiming_normal_(model.conv1.weight, mode='fan_out', nonlinearity='linear')
    nn.init.ones_(model.bn1.weight)
    nn.init.zeros_(model.bn1.bias)
    # Tête de régression dégelée
...
def run_experiment(target: str, exp: str):
    MODEL_NAME = 'EEGNet_LayerReplacement'
    print(f"\n{'='*65}")
    ...
    with open(outdir / 'metrics.txt', 'w', encoding='utf-8') as f:
        f.write(f"EEGNet Layer Replacement – {target.upper()} – Exp. {exp}\n")
    ...
    torch.save(model.state_dict(), moddir / f"EEGNet_LR_{target}_{exp}_best.pt")
    print(f"  → {outdir}")
    return full_m
