"""
NeuroCap – EEGNet Feature Extraction (Prabhakar 2022, TL-2)
Stratégie : geler toutes les couches sauf FC — le réseau sert d'extracteur de features fixe
...
def plot_confusion(yt, yp, outdir, exp):
    ...
    plt.title(f'EEGNet Feature Extraction – Exp. {exp}')
...
def plot_roc(yt, yp, outdir, exp):
    ...
    plt.title(f'EEGNet Feature Extraction – ROC (Exp.{exp})')
...
def plot_proba(yt, yp, outdir, exp):
    ...
    plt.title(f'EEGNet Feature Extraction – Probas (Exp.{exp})')
...
def run_experiment(exp):
    print("="*70)
    print(f"NeuroCap – EEGNet Feature Extraction (Prabhakar 2022) – Exp. {exp}")
    print("="*70)
    ...
    metrics = {
        ...
        'model': 'EEGNet_FeatureExtraction', 'exp_name': exp,
    ...
    print(f"\n  ✅ Résultats EEGNet Feature Extraction – Exp. {exp} :")
    ...
    # Sauvegarde
    out_dir = OUTPUT_BASE / "EEGNet_FeatureExtraction" / f"exp_{exp}"
    model_dir = MODEL_BASE / "EEGNet_FeatureExtraction"
    ...
    with open(out_dir/'metrics.txt', 'w') as f:
        f.write(f"EEGNet Feature Extraction – Exp. {exp}\n")
    ...
    torch.save(m.state_dict(), model_dir/f"EEGNet_FE_{exp}_best.pt")
    print(f"  Modèle sauvegardé : EEGNet_FE_{exp}_best.pt\n✅ Terminé.\n")
