"""
NeuroCap – EEGNet Full Fine-Tuning (Wan 2021, TL-1)
...
def apply_full_finetuning(model):
    """Dégeler TOUTES les couches avec learning rate réduit (Wan 2021)."""
...
def plot_confusion(yt, yp, outdir, exp):
    ...
    plt.title(f'EEGNet Full Fine-Tuning – Exp. {exp}')
...
def plot_roc(yt, yp, outdir, exp):
    ...
    plt.title(f'EEGNet Full Fine-Tuning – ROC (Exp.{exp})')
...
def plot_proba(yt, yp, outdir, exp):
    ...
    plt.title(f'EEGNet Full Fine-Tuning – Probas (Exp.{exp})')
...
def run_experiment(exp):
    print("="*70)
    print(f"NeuroCap – EEGNet Full Fine-Tuning (Wan 2021) – Exp. {exp}")
    print("="*70)
    ...
    metrics = {
        ...
        'model': 'EEGNet_FullFT', 'exp_name': exp,
    ...
    print(f"\n  ✅ Résultats EEGNet Full Fine-Tuning – Exp. {exp} :")
    ...
    # Sauvegarde
    out_dir = OUTPUT_BASE / "EEGNet_FullFT" / f"exp_{exp}"
    model_dir = MODEL_BASE / "EEGNet_FullFT"
    ...
    with open(out_dir/'metrics.txt', 'w') as f:
        f.write(f"EEGNet Full Fine-Tuning – Exp. {exp}\n")
    ...
    torch.save(m.state_dict(), model_dir/f"EEGNet_FFT_{exp}_best.pt")
    print(f"  Modèle sauvegardé : EEGNet_FFT_{exp}_best.pt\n✅ Terminé.\n")
