"""
NeuroCap – EEGNet Full Fine-Tuning RÉGRESSION (Wan 2021, TL-1)
==============================================================
Stratégie  : dégeler TOUTES les couches avec LR réduit (1e-4).
...
Sorties      : reports/Regression/TL/EEGNet_FullFT/{target}/{exp}/

Référence : Wan et al. (2021) — IEEE TNSRE
  "EEG-based emotion recognition using a meta-learning network with
   attention mechanisms" — full fine-tuning avec LR 10× réduit.
"""
...
def _load_best_pretrained(target: str, preferred_exp: str):
    ...
    """
    R2_MIN = 0.05   # seuil en dessous duquel on considère le modèle non pertinent
    ...
    # Utiliser le pré-entraîné de l'exp courante si R² acceptable
    if pt_preferred.exists() and r2_preferred >= R2_MIN:
        return pt_preferred, preferred_exp

    # Sinon chercher la meilleure exp disponible
...
# ── Stratégie TL-1 : Full Fine-Tuning ─────────────────────────────────────────
def apply_full_finetuning(model):
    """Dégeler TOUTES les couches — Wan 2021."""
...
def run_experiment(target: str, exp: str):
    ...
    # Modèle pré-entraîné — avec fallback sur la meilleure expérience disponible
    ...
    out_dict = {
        **full_m,
        ...
        'strategy': 'Full Fine-Tuning (Wan 2021)',
    ...
    with open(outdir / 'metrics.txt', 'w', encoding='utf-8') as f:
        f.write(f"EEGNet Full Fine-Tuning – {target.upper()} – Exp. {exp}\n")
    ...
    torch.save(model.state_dict(), moddir / f"EEGNet_FFT_{target}_{exp}_best.pt")
    print(f"  → {outdir}")
    return full_m
