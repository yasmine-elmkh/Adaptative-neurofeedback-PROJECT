...
def save_augmented_datasets(datasets, X_val, y_val, X_test, y_test,sid_train, sid_val, sid_test,
                             outdir=Path(__file__).resolve().parent.parent / 'data' / 'Augmentation' / 'outputs_augmentation' / 'datasets_augmented'):
    """
    ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
    ...
    """
    Sauvegarde des datasets augmentés AVEC les subject_ids.
    """
    ...
    for exp_name, (X_tr, y_tr) in datasets.items():
        # Calcul du facteur d'augmentation : combien de fois les données originales ont été répétées
        n_orig = len(sid_train)
        n_aug = len(y_tr)
        if n_aug % n_orig == 0:
            factor = n_aug // n_orig
            sid_tr = np.tile(sid_train, factor)
        else:
            # Si non divisible, on répète puis on tronque (cas rare, mais on gère)
            factor = (n_aug + n_orig - 1) // n_orig
            sid_tr = np.tile(sid_train, factor)[:n_aug]
            print(f"    ⚠️ Exp. {exp_name} : répétition non exacte, factor approximatif {factor}")

        np.save(os.path.join(outdir, f'X_train_{exp_name}.npy'), X_tr)
        np.save(os.path.join(outdir, f'y_train_{exp_name}.npy'), y_tr)
        np.save(os.path.join(outdir, f'subject_ids_train_{exp_name}.npy'), sid_tr)
        print(f"    ✓ Exp. {exp_name} : X {X_tr.shape}, y {y_tr.shape}, IDs {sid_tr.shape} (factor={n_aug//n_orig if n_aug%n_orig==0 else '~'}{factor})")

    # Sauvegarde val et test (non augmentés)
    np.save(os.path.join(outdir, 'X_val.npy'), X_val)
    np.save(os.path.join(outdir, 'y_val.npy'), y_val)
    np.save(os.path.join(outdir, 'subject_ids_val.npy'), sid_val)

    np.save(os.path.join(outdir, 'X_test.npy'), X_test)
    np.save(os.path.join(outdir, 'y_test.npy'), y_test)
    np.save(os.path.join(outdir, 'subject_ids_test.npy'), sid_test)

    # Sauvegarde du fichier global subject_ids_train.npy
    np.save(os.path.join(outdir, 'subject_ids_train.npy'), sid_train)

    print(f"    ✓ Tous les fichiers (y compris subject_ids) sauvegardés dans {outdir}")
    return outdir
...
# ─────────────────────────────────────────────────────────────────────────────
    ...
    """
    ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
    PIPELINE PRINCIPALE APRES LE MERGE — Augmentation correcte et professionnelle
    ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
    ...
    """
  # ─────────────────────────────────────────────────────────────────────────────
# NOUVELLE FONCTION PRINCIPALE POUR DONNÉES FUSIONNÉES
# ─────────────────────────────────────────────────────────────────────────────

def run_augmentation_on_merged(merged_dir=Path(__file__).resolve().parent.parent.parent / 'data' / 'Merge_datasets' / 'datasets_merged',
                                outdir=Path(__file__).resolve().parent.parent.parent / 'data' / 'Augmentation' / 'outputs_augmentation',
                                save_dir=None):
    """
    Version pour les données fusionnées (X_merged.npy, y_merged.npy, subject_ids_merged.npy)
    """
    os.makedirs(outdir, exist_ok=True)
    if save_dir is None:
    ...
    print('=' * 65)
    print('NeuroCap — Pipeline d\'Augmentation EEG Fp2 (sur données fusionnées)')
    print(f'fs={FS} Hz | Epoch {EPOCH_S}s | Overlap {OVERLAP*100:.0f}%')
    print('=' * 65)

    # Chargement des données fusionnées
    print('\n[LOAD] Chargement des données fusionnées...')
    X = np.load(os.path.join(merged_dir, 'X_merged.npy'))
    y = np.load(os.path.join(merged_dir, 'y_merged.npy'))
    sids = np.load(os.path.join(merged_dir, 'subject_ids_merged.npy'))
    print(f'    X shape : {X.shape}, y shape : {y.shape}, {len(np.unique(sids))} sujets')
    print(f'    Labels : concentration (0) = {np.sum(y==0)} epochs, stress (1) = {np.sum(y==1)} epochs')

    # (Optionnel : normalisation z-score si pas déjà faite – décommentez si nécessaire)
    # X = np.array([zscore_normalize(ep) for ep in X])

    # Split par sujet
    print('\n[SPLIT] Séparation par sujet (évite data leakage)...')
    (X_train, y_train, sid_train,
     X_val,   y_val,   sid_val,
     X_test,  y_test,  sid_test) = split_by_subject(X, y, sids, test_ratio=0.15, val_ratio=0.15, seed=42)
    ...
    for grp_name, copies in [('Exp.B', copies_B), ('Exp.C', copies_C), ('Exp.D', copies_D)]:
        ...
        for ratio, r in res.items():
            ok = '✓' if r['ok'] else '✗'
            print(f'    {ok} {grp_name} | {ratio} : orig={r["orig"]:.3f} → aug={r["aug"]:.3f} | Δ={r["delta_%"]:.1f}%')

    # Sauvegarde
    print(f'\n[SAVE] Sauvegarde dans {save_dir}...')
    save_augmented_datasets(datasets, X_val, y_val, X_test, y_test, sid_train, sid_val, sid_test, outdir=save_dir)
    ...
    print(f'    ✓ 9 figures générées dans {outdir}')

    print(f'\n✅ Pipeline terminé — datasets sauvegardés dans : {save_dir}')
    return datasets, X_val, y_val, X_test, y_test


if __name__ == '__main__':
    # Lancer le pipeline sur les données fusionnées
    run_augmentation_on_merged(
        merged_dir=Path(__file__).resolve().parent.parent.parent / 'data' / 'Merge_datasets' / 'datasets_merged',
        outdir=Path(__file__).resolve().parent.parent.parent / 'reports' / 'Augmentation' / 'outputs_augmentation',
        save_dir=Path(__file__).resolve().parent.parent.parent / 'data' / 'Augmentation' / 'datasets_augmented'
    )