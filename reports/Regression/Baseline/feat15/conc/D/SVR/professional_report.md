# NeuroCap — Rapport Professionnel de Performance
**Target :** `conc`  |  **Modèle :** `SVR`  |  **Exp :** `D`  |  **Date :** 2026-06-21 16:29


## Niveau 1 — Métriques Cliniques


### Régression

| Métrique | Valeur | Interprétation |
|---|---|---|
| MAE | 2.5985 | Erreur absolue moyenne (0-10) |
| RMSE | 3.1265 | Erreur quadratique moyenne |
| R² | -0.0190 | 1=parfait, 0=moyenne, <0=pire |

### Classification Binée (Low:0-5 / High:5-10)

| Métrique | Valeur | Baseline aléatoire |
|---|---|---|
| AUC ROC | 0.6168 | 0.500 |
| PR-AUC | 0.6269 | 0.535 |
| Sensitivity (TPR) | 0.7819 | 0.500 |
| Specificity (TNR) | 0.3935 | 0.500 |
| PPV (Precision) | 0.5971 | — |
| NPV | 0.6108 | — |
| Balanced Accuracy | 0.5877 | 0.500 |
| MCC | 0.1909 | 0.000 |
| G-Mean | 0.5547 | 0.500 |
| F1 macro | 0.5779 | 0.500 |
| LR+ | 1.289 | >10 = très utile |
| LR− | 0.554 | <0.1 = très utile |
| Cohen κ | 0.1795 | 0.000 |
| Brier Score | 0.3103 | < 0.20 objectif |

## Niveau 2 — Intervalles de Confiance (Bootstrap 95%)

| Métrique | Valeur | CI 95% | Significatif |
|---|---|---|
| AUC ROC | 0.6205 | [0.6005, 0.6416]  ✅ |
| F1 macro | 0.5594 | [0.5408, 0.5784]  ✅ |
| Sensitivity | 0.4406 | [0.4143, 0.4663]  — |
| Specificity | 0.6970 | [0.6720, 0.7216]  — |
| MCC | 0.1422 | [0.1053, 0.1799]  — |
| R² | -0.0195 | [-0.0569, 0.0207]  — |

### Tests de Permutation

| Métrique | Observé | p-value | Significatif |
|---|---|---|
| R² | -0.0190 | p=0.0000 | ✅ p<0.05 |
| AUC ROC | 0.6210 | p=0.0000 | ✅ p<0.05 |

## Niveau 3 — Qualité Clinique & Incertitude


### Calibration

| Métrique | Valeur | Objectif |
|---|---|---|
| ECE | 0.2599 | < 0.05 |
| MCE | 0.3776 | < 0.10 |
| Brier Score | 0.3156 | < 0.20 |
| Log-Loss | 1.0552 | minimiser |

### Bland-Altman (Accord Clinique)

| Métrique | Valeur | Interprétation |
|---|---|---|
| Biais | -0.0307 | proche 0 = pas de biais systématique |
| LoA lower | -6.1595 | limite inférieure d'accord |
| LoA upper | +6.0980 | limite supérieure d'accord |
| LoA width | ±6.1288 | < ±2 pts : excellent |
| % dans LoA | 96.6% | ≥ 95% requis |
| Accord clinique | ✅ OUI |  |

### ICC (Intraclass Correlation)

| ICC(2,1) | 0.1654 | insufficient (<0.50) |

## Niveau 4 — Robustesse & Stabilité


### Stabilité LOSO (Coefficient de Variation)

| Métrique | Moyenne | Std | CV% | Interprétation |
|---|---|---|
| R² | -0.0749 | 0.2536 | 338.5% | 🔴 unstable |
| AUC ROC | 0.6276 | 0.0685 | 10.9% | 🟢 stable |
| MAE | 2.6126 | 0.3027 | 11.6% | 🟢 stable |

## Niveau 5 — Tests Statistiques Avancés (DCA & DeLong)


### AUC — Méthode DeLong (CI analytique exact)

| | Valeur |
|---|---|
| AUC | 0.6210 |
| CI 95% | [0.6006, 0.6415] |
| p-value | 0.000000 |
| Significatif | ✅ OUI |

### Decision Curve Analysis (Utilité Clinique)

**Modèle cliniquement utile : ✅ OUI**

Zone d'utilité : seuil de décision entre 0.46 et 0.59


## Contexte & Recommandations

| Conditions | Valeur |
|---|---|
| Nb électrodes | 1 (FP2 — préfrontal) |
| Protocole validation | LOSO cross-sujet (strict) |


🟡 **Performance modérée. Recommander collecte de données supplémentaires.**


---
*Rapport généré automatiquement par NeuroCap metrics_professional.py — 2026-06-21 16:29*
