"""
NeuroCap — Génération rapport session via Claude API
"""
...
async def generate_session_report(session_data: dict) -> Optional[str]:
    """
    Génère un rapport de séance de neurofeedback en français via Claude API.
...
Ton : professionnel mais encourageant. Ne commence pas par "Dans cette séance"."""
        ...
        logger.error("Erreur génération rapport Claude : %s", e)
        return None
...
async def generate_bilan_recommendation(
    ...
    """
    Génère une recommandation de bilan (S5/S10/S15) via Claude API.
    """
    try:
        client = anthropic.AsyncAnthropic()
...
Ton : professionnel, motivant, précis."""
        ...
        logger.error("Erreur génération bilan Claude : %s", e)
        return None
...
async def generate_media_guide(
    ...
    try:
        ...
        if mtype in ("image", "video"):
            ...
            # Priorité root level, puis features JSONB
            ...
            # Descripteurs spécifiques aux images fixes
            if mtype == "image":
                ...
                # Style : géométrique / abstrait / naturel
...
- N'explique pas, ne justifie pas, donne juste le guide."""
        ...
        logger.error("Erreur génération guide média Claude : %s", e)
        return None
