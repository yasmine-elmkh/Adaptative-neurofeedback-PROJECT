...
export async function getMediaByFilename(filename) {
  ...
  const { data, error } = await supabase
    ...
    .single()
  if (error) { console.warn('[Supabase] getMediaByFilename:', error.message); return null }
  return data
}
...
export async function getRecommandation(eegState, mediaType = null) {
  ...
  if (!medias || medias.length === 0) throw new Error(`Aucun média pour ${cat}`)
...
export async function updateMediaWeights(mediaId, eegState, liked) {
  ...
  const cat = EEG_TO_CAT[eegState] || 'transition'
  const { data: media } = await supabase.from('medias').select('features').eq('id', mediaId).single()
  if (!media) return
