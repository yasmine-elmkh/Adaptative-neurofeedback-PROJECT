// FeedbackJustification.jsx — Guide IA dynamique avec tips cycliques
...
export default function FeedbackJustification({ mediaId, type, state, userId }) {
  ...
  // Charge la justification depuis l'API quand le mediaId change
  useEffect(() => {
  ...
  // Réinitialise l'index quand type ou données changent
  useEffect(() => { setTipIdx(0) }, [apiData, type])

  // Cycle automatique des tips toutes les 4.5 s avec fade
  ...
  return (
    <div style={{
      ...
      background: 'rgba(184,123,158,0.08)',
      border: '1px solid rgba(184,123,158,0.25)',
    }}>
      {/* En-tête */}
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
        ...
        {isAI ? (
          <span style={{
            ...
            color: '#7BA8C4', background: 'rgba(123,168,196,0.12)',
            border: '1px solid rgba(123,168,196,0.3)',
      ...
      {/* Justification texte */}
      <div style={{ fontSize: 11.5, color: '#2B2A4A', lineHeight: 1.65, marginBottom: 10 }}>
        {justText}
      </div>
      ...
      {tips.length > 0 && (
        <div style={{
          ...
          border: '1px solid rgba(184,123,158,0.2)',
          padding: '8px 12px',
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 3, flexShrink: 0 }}>
            {tips.map((_, i) => (
              <div key={i} style={{
                width: 3,
                height: i === tipIdx ? 14 : 5,
                borderRadius: 2,
                background: i === tipIdx ? '#B87B9E' : 'rgba(184,123,158,0.25)',
                transition: 'all .4s',
              }} />
            ))}
          </div>
          ...
          {/* Texte du tip */}
          <div style={{
            fontSize: 12, color: '#2B2A4A', lineHeight: 1.55, flex: 1,
            opacity: visible ? 1 : 0,
          ...
          <button
            ...
            style={{
              ...
              fontSize: 16, color: 'rgba(184,123,158,0.5)',
