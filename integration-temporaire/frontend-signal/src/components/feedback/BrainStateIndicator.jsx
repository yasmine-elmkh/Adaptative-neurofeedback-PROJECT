// BrainStateIndicator.jsx — Palette Bleu Mauve
const STATE_THEME = {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 14 }}>
      <div style={{
        ...
        <style>{`@keyframes blink-dot{0%,100%{opacity:1;transform:scale(1)}50%{opacity:.6;transform:scale(.8)}}`}</style>
        <span style={{ fontSize: 15 }}>{theme.icon}</span>
        {theme.label}
      </div>
      {confidence != null && (
        <span style={{ fontSize: 10, color: '#9A8BAE', fontFamily: "'DM Mono',monospace" }}>
